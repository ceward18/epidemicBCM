...
for (i in batchIdx) {
    ...
    if (i == batchIdx[1]) {
        ...
        postPredFit <- cbind.data.frame(postSummaries$postPredFit, modelInfo)
        ...
        postPredFit <- rbind.data.frame(postPredFit, 
...
saveRDS(postPredFit, paste0('./Output/postPredFitBatch', idx, '.rds'))
