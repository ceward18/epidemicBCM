...
Tables 6 - 11 and Figures 12 - 23 show a sampling of trace plots and estimates of the alarm function or $\beta_t$ over time for models that
...
grPrintTrans <- t(grPrint[,c('param', 'gr')])
colnames(grPrintTrans) <- NULL
rownames(grPrintTrans) <- c('Parameter', 'Gelman-Rubin')
...
```{r}
#| fig-cap: "Posterior mean and 95% credible intervals for the beta[t] function from each chain. Data: Power, Model: beta[t],  Smoothing: 14 days, Simulation: 7."
#| fig-height: 2.4
#| fig-width: 7
...
```{r}
#| fig-cap: "Posterior mean and 95% credible intervals for the beta[t] function from each chain. Data: Threshold, Model: beta[t], Smoothing: 14 days, Simulation: 1."
#| fig-height: 2.4
#| fig-width: 7
...
```{r}
#| fig-cap: "Posterior mean and 95% credible intervals for the beta[t] function from each chain. Data: Hill, Model: beta[t], Smoothing: 14 days, Simulation: 1"
#| fig-height: 2.4
#| fig-width: 7
...
```{r}
#| fig-cap: "Posterior mean and 95% credible intervals for the beta[t] function from each chain. Data: Threshold, Model: beta[t], Smoothing: 30 days, Simulation: 1."
#| fig-height: 2.4
#| fig-width: 7
...
\begin{center}
\begin{table*}[ht!]%
\caption{Priors used to fit models in the DRC Ebola data analysis. $\Gamma$ denotes the gamma distribution with shape and rate parameters. $\min(x)$ and $\max(x)$ are the minimum and maximum observed value of the cumulative incidence during the epidemic, respectively. \vspace{3mm}
\label{tabSimParams}}
\centering
\begin{tabular}{ll}
\hline
Model & Priors  \\ \hline \\[-0.35cm]
Power alarm & \begin{tabular}[c]{@{}l@{}}$\beta \sim \Gamma(0.1, 0.1)$\\ $\lambda \sim \Gamma(20, 100)$\\ $\gamma \sim \Gamma(20, 140)$\\ $k \sim \Gamma(0.1, 0.1)$\end{tabular}  \\ \hline \\[-0.35cm]
Threshold alarm & \begin{tabular}[c]{@{}l@{}}$\beta \sim \Gamma(0.1, 0.1)$\\ $\lambda \sim \Gamma(20, 100)$\\ $\gamma \sim \Gamma(20, 140)$\\ $\delta \sim \text{Uniform}(0, 1)$\\ $H \sim \text{Uniform}(\min(x), \max(x))$\end{tabular}  \\ \hline \\[-0.35cm]
Hill alarm & \begin{tabular}[c]{@{}l@{}}$\beta \sim \Gamma(0.1, 0.1)$\\ $\lambda \sim \Gamma(20, 100)$\\ $\gamma \sim \Gamma(20, 140)$\\ $\delta \sim \text{Uniform}(0, 1)$\\ $\nu \sim \text{Uniform}(0, 50)$\\ $x_0 \sim \text{Uniform}(\min(x), \max(x))$\end{tabular}  \\ \hline \\[-0.35cm]
Spline alarm & \begin{tabular}[c]{@{}l@{}}$\beta \sim \Gamma(0.1, 0.1)$\\ $\lambda \sim \Gamma(20, 100)$\\ $\gamma \sim \Gamma(20, 140)$\\ $b_{1-3} \sim N(0, 100^2)$\\ $\text{knot}_{1-2} \sim \text{Uniform}(\min(x), \max(x))$\end{tabular}  \\ \hline \\[-0.35cm]
Gaussian process alarm & \begin{tabular}[c]{@{}l@{}}$\beta \sim \Gamma(0.1, 0.1)$\\ $\lambda \sim \Gamma(20, 100)$\\ $\gamma \sim \Gamma(20, 140)$\\ $\sigma \sim \Gamma(150, 50)$\\ $l \sim \text{inv}\Gamma(a, b)$\end{tabular}  \\ \hline \\[-0.35cm]
No behavioral change & \begin{tabular}[c]{@{}l@{}}$\beta \sim \Gamma(0.1, 0.1)$\\ $\lambda \sim \Gamma(20, 100)$\\ $\gamma \sim \Gamma(20, 140)$ \end{tabular} \\ \hline \\[-0.35cm] 
Pre-specified intervention & \begin{tabular}[c]{@{}l@{}}$\beta_1 \sim N(0, 4^2)$\\ $\beta_2 \sim N(0, 4^2)$\\ $\lambda \sim \Gamma(20, 100)$\\ $\gamma \sim \Gamma(20, 140)$ \end{tabular} \\ \hline \\[-0.35cm] 
Flexible $\beta_t$ & \begin{tabular}[c]{@{}l@{}}$\beta \sim \Gamma(0.1, 0.1)$\\ $\lambda \sim \Gamma(20, 100)$\\ $\gamma \sim \Gamma(20, 140)$\\ $b_{1-4} \sim N(0, 100^2)$\\ $\text{knot}_{1-3} \sim \text{Uniform}(1, \tau)$\end{tabular} \\ \hline \\
\end{tabular}
\end{table*}
\end{center}
```

All models were run for 250,000 iterations after a burn-in period of 250,000 iterations. Thinning of every 20th iteration was applied.
...
paramsPostAll <- readRDS('./data_analysis_ebola/results/paramsPostAll.rds')

# more interpretable on scale 1/lambda
rateEParams <- paramsPostAll[paramsPostAll$param == 'rateE',]
meanEParams <- rateEParams
meanEParams$mean <- 1/meanEParams$mean
meanEParams$lower <- 1/meanEParams$lower
meanEParams$upper <- 1/meanEParams$upper
meanEParams$param <- 'meanE'

# more interpretable on scale 1/gamma
...
paramsPostAll <- rbind.data.frame(paramsPostAll, meanEParams, meanIParams)
paramsPostAll <- paramsPostAll[-which(paramsPostAll$param %in% c('rateE', 'rateI')),]

paramsPostAll$param <- factor(paramsPostAll$param, 
                              levels = c("beta", "transParams[1]", "transParams[2]",
                                         "meanE", "meanI",
                                         "k",  "delta", "H", 
                                         "nu", "x0",
                                         "l",  "sigma",
...
# fix H (estimated on scale 1/N)
paramsPostAll$mean[paramsPostAll$param == 'H'] <- 
    paramsPostAll$mean[paramsPostAll$param == 'H'] * 5363500
paramsPostAll$lower[paramsPostAll$param == 'H'] <- 
    paramsPostAll$lower[paramsPostAll$param == 'H'] * 5363500
paramsPostAll$upper[paramsPostAll$param == 'H'] <- 
    paramsPostAll$upper[paramsPostAll$param == 'H'] * 5363500


# format k to 5 decimal places
paramsPostAll$mean[paramsPostAll$param == 'k'] <-
    sprintf('%.5f', paramsPostAll$mean[paramsPostAll$param == 'k'])
paramsPostAll$lower[paramsPostAll$param == 'k'] <- 
    sprintf('%.5f', paramsPostAll$lower[paramsPostAll$param == 'k'])
paramsPostAll$upper[paramsPostAll$param == 'k'] <- 
    sprintf('%.5f', paramsPostAll$upper[paramsPostAll$param == 'k'])


# format everything except k to 2 decimals
paramsPostAll$mean[!grepl('k$', paramsPostAll$param)] <-
    sprintf('%.2f', as.numeric(paramsPostAll$mean[!grepl('k$', 
                                                         paramsPostAll$param)]))
paramsPostAll$lower[!grepl('k$', paramsPostAll$param)] <- 
    sprintf('%.2f', as.numeric(paramsPostAll$lower[!grepl('k$',
                                                          paramsPostAll$param)]))
paramsPostAll$upper[!grepl('k$', paramsPostAll$param)] <- 
    sprintf('%.2f', as.numeric(paramsPostAll$upper[!grepl('k$',
                                                          paramsPostAll$param)]))
...
```{r}
#| tbl-cap: Posterior mean and 95% credible intervals for all parameters in the power alarm model

postParamTableEbola(paramsPostAll, 'power')
```

##### Threshold alarm

```{r}
#| tbl-cap: Posterior mean and 95% credible intervals for all parameters in the threshold alarm model

postParamTableEbola(paramsPostAll, 'thresh')
```

##### Hill alarm

```{r}
#| tbl-cap: Posterior mean and 95% credible intervals for all parameters in the Hill alarm model

postParamTableEbola(paramsPostAll, 'hill')
```

##### Spline alarm

```{r}
#| tbl-cap: Posterior mean and 95% credible intervals for all parameters in the spline alarm model

postParamTableEbola(paramsPostAll, 'spline', scale = T)
```

##### Gaussian process alarm

```{r}
#| tbl-cap: Posterior mean and 95% credible intervals for all parameters in the Gaussian process alarm model

postParamTableEbola(paramsPostAll, 'gp')
```

##### No Behavioral Change

```{r}
#| tbl-cap: Posterior mean and 95% credible intervals for all parameters in the model with no behavioral change

postParamTableEbola(paramsPostAll, 'basic')
```

##### Pre-specified intervention

```{r}
#| tbl-cap: Posterior mean and 95% credible intervals for all parameters in the model with a pre-specified intervention
...
```{r}
#| tbl-cap: Posterior mean and 95% credible intervals for all parameters in the flexible beta[t] model

postParamTableEbola(paramsPostAll, 'betatSpline', scale = T)
```
...
# fix H (estimated on scale 1/N)
paramsPostAll$mean[paramsPostAll$param == 'H'] <- 
    paramsPostAll$mean[paramsPostAll$param == 'H'] * 8804190
paramsPostAll$lower[paramsPostAll$param == 'H'] <- 
    paramsPostAll$lower[paramsPostAll$param == 'H'] * 8804190
paramsPostAll$upper[paramsPostAll$param == 'H'] <- 
    paramsPostAll$upper[paramsPostAll$param == 'H'] * 8804190

# format H with no decimals
paramsPostAll$mean[paramsPostAll$param == 'H'] <- 
    round(paramsPostAll$mean[paramsPostAll$param == 'H'])
paramsPostAll$lower[paramsPostAll$param == 'H'] <- 
    round(paramsPostAll$lower[paramsPostAll$param == 'H'])
paramsPostAll$upper[paramsPostAll$param == 'H'] <- 
    round(paramsPostAll$upper[paramsPostAll$param == 'H'])
...
# format everything except S0, H and k to 2 decimals
paramsPostAll$mean[!grepl('SIR_init\\[1\\]|k$|H$', paramsPostAll$param)] <-
    sprintf('%.2f', as.numeric(paramsPostAll$mean[!grepl('SIR_init\\[1\\]|k$|H$', 
                                                         paramsPostAll$param)]))
paramsPostAll$lower[!grepl('SIR_init\\[1\\]|k$|H$', paramsPostAll$param)] <- 
    sprintf('%.2f', as.numeric(paramsPostAll$lower[!grepl('SIR_init\\[1\\]|k$|H$',
                                                          paramsPostAll$param)]))
paramsPostAll$upper[!grepl('SIR_init\\[1\\]|k$|H$', paramsPostAll$param)] <- 
    sprintf('%.2f', as.numeric(paramsPostAll$upper[!grepl('SIR_init\\[1\\]|k$|H$',
                                                          paramsPostAll$param)]))
...
```{r}
#| tbl-cap: Posterior mean and 95% credible intervals for all parameters in the flexible beta[t] models.
